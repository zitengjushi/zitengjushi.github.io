...
class Spider(Spider):
    ...
    def _extract_video(self, element, category_id=None):
        ...
        try:
            ...
            # 图片解密处理
            if pic and 'nmmmz.rclwp.cn' in pic:
                try:
                    # 尝试解密图片
                    decrypted_pic = self._decrypt_image(pic)
                    if decrypted_pic:
                        pic = decrypted_pic
                except Exception as e:
                    self.log(f"图片解密失败: {str(e)}")
                    # 如果解密失败，使用默认图片
                    pic = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAJcEhZcwAADsQAAA7EAZUrDhsAAAANSURBVBhXYzh8+PB/AAffA0nNPuCLAAAAAElFTkSuQmCC"
    ...
    def _decrypt_image(self, encrypted_url):
        """解密图片URL - 根据网页中的解密逻辑"""
        try:
            # 网页中的解密逻辑：使用密钥"2019ysapp7527"进行异或解密
            key = b"2019ysapp7527"
            
            # 下载加密的图片数据
            response = self.fetch(encrypted_url, headers=self.headers)
            if not response or response.status_code != 200:
                self.log(f"图片下载失败: {encrypted_url}")
                return None
            ...
            # 转换为base64 data URL
            mime_type = self._get_image_mime_type(encrypted_url)
    ...
    def _get_image_mime_type(self, url):
        """根据URL获取图片MIME类型"""
